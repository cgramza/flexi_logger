...
impl LwHandle {
    fn new(config: &LogConfig) -> Result<LwHandle, FlexiLoggerError> {
        ...
        lwh.mount_linewriter(&config.suffix, &config.timestamp, config.print_message);
        Ok(lwh)
    }
...
fn get_next_rotate_idx(s_filename_base: &String, o_suffix: &Option<String>) -> usize {
    ...
    match paths {
        Err(e) => {
            panic!("Is this ({}) really a directory? Listing failed with {}",
                   fn_pattern,
                   e); // FIXME
        }
        Ok(it) => {
...
impl FlexiLogger {
    pub fn new(loglevelspec: Option<String>,
               config: LogConfig)
    ...
    fn log(&self, record: &LogRecord) {
        ...
        if self.config.log_to_file {
            ...
            if lwh.use_rotating && (lwh.written_bytes > self.config.rotate_over_size.unwrap()) {
                ...
                lwh.written_bytes = 0;
                lwh.rotate_idx += 1;
                lwh.mount_linewriter(&self.config.suffix,
                                     &self.config.timestamp,
                                     self.config.print_message);
            }
            if let Some(ref mut lw) = lwh.olw {
                &lw.write(msgb).unwrap_or_else(|e| {
                    panic!("File logger: write failed with {}", e);
                });//FIXME
                if lwh.use_rotating {
    ...
    match result {
        Ok(fl) => {
            ...
            log::set_logger(|max_level| {
                ...
                .map_err(|e| {
                    FlexiLoggerError::new(format!("Logger initialization failed due to {}", e))
                })
        }
        Err(e) => Err(e),
    }
...
fn parse_logging_spec(spec: &str) -> (Vec<LogDirective>, Option<Regex>) {
    ...
    if parts.next().is_some() {
        print_err!("warning: invalid logging spec '{}', ignoring it (too many '/'s)",
                   spec);
        return (dirs, None);
    }
